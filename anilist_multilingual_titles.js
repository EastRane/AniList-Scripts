...
(function() {
	...
	async function fetchAniListData(mediaId) {
		...
		if (!data) throw new Error('No media data from AniList');
	...
	async function fetchShikimoriRussian(idMal, shikiType) {
		...
		try {
			...
			return shikiData.russian || null;
		} catch (e) {
			console.debug(`[Triple Title] Shikimori error for ${shikiType}/${idMal}`, e);
			return null;
		}
	}
	...
	async function updateTitle() {
		...
		try {
			...
			let romaji, english, idMal, russian;

			if (cached) {
				romaji = cached.romaji || null;
				english = cached.english || null;
				idMal = cached.idMal || null;
				russian = cached.russian || null;

				const hasAllTitles = romaji && english && russian !== undefined;

				if (!hasAllTitles) {
					console.debug('[Triple Title] Incomplete cache, fetching missing data');

					if (!romaji || !english || !idMal) {
					...
					if (russian === null && idMal) {
						russian = await fetchShikimoriRussian(idMal, media.shikiType);
					}
