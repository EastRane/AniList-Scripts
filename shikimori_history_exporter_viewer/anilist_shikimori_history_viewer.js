...
(function () {
    ...
    async function loadShikiHistory() {
        ...
        try {
            if (!location.pathname.includes("/social")) { loadingInProgress = false; return; }
            const match = location.pathname.match(/\/(anime|manga|ranobe|novel)\/(\d+)/);
            if (!match) { loadingInProgress = false; return; }
            ...
            const gql = {
                query: `
                    query ($id: Int!) {
                        Media (id: $id) { idMal title { romaji } }
                    }`,
            ...
            if (!malId) { loadingInProgress = false; return; }

            const shikiEvents = EVENTS_BY_SHIKI_ID[String(malId)];
            if (!shikiEvents || !shikiEvents.length) { loadingInProgress = false; return; }

            await waitForElement("#activityTimeline", 10000);
            const timeline = document.getElementById("activityTimeline");
            if (!timeline) { loadingInProgress = false; return; }

            renderEventsBlock(shikiEvents, timeline);
            loadingInProgress = false;
        } catch (err) { console.error(err); loadingInProgress = false; }
    }
    ...
    function waitForElement(selector, timeout = 5000) {
        return new Promise(resolve => {
            ...
            const mo = new MutationObserver(() => {
                ...
                if (e && !resolved) { resolved = true; mo.disconnect(); resolve(e); }
            });
            mo.observe(document.body, { childList: true, subtree: true });

            setTimeout(() => { if (!resolved) { resolved = true; mo.disconnect(); resolve(null); } }, timeout);
        });
    }

    function formatTimelineDate_isoInput(dateIso) {
        const dateObj = new Date(dateIso);
        const short = dateObj.toLocaleDateString("en-UK", { weekday: "short", day: "2-digit", month: "short", year: "numeric" });
    ...
    function renderEventsBlock(events, timelineElement) {
        ...
        for (const [dateStr, action] of events) {
            ...
            const { short, full } = formatTimelineDate_isoInput(dateStr);
    ...
    function handleUrlChange() {
        ...
        lastPath = cur;
        if (cur.includes("/social")) setTimeout(() => { if (!document.getElementById("shikiHistoryBlock")) loadShikiHistory(); }, 150);
    }
    ...
    if (location.pathname.includes("/social")) setTimeout(() => { loadShikiHistory(); }, 200);
